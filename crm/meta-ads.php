...
<div class="flex flex-wrap justify-between items-center gap-4 mb-8">
    <div>
        ...
        <p class="text-slate-400 text-xs mt-1">Connect Marketing API and inspect clicks, UTM campaigns and incoming lead details.</p>
    </div>
...
<div class="bg-blue-600 text-white p-5 rounded-2xl shadow-sm border border-blue-700 mb-8 flex items-center justify-between flex-wrap gap-4">
    ...
    <?php foreach ($mockCampaigns as $camp): ?>
        <div class="col-lg-4">
            <div class="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm relative overflow-hidden">
                <span class="absolute top-4 right-4 text-[9px] font-extrabold px-2 py-0.5 rounded <?php echo $camp['status'] === 'ACTIVE' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-slate-100 text-slate-500'; ?>">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <span class="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block mb-0.5">Clicks Tracked</span>
                        <span class="font-black text-slate-800 text-lg"><?php echo $camp['clicks']; ?></span>
                    </div>
                    <div>
                        <span class="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block mb-0.5">Leads Captured</span>
                        <span class="font-black text-brand-500 text-lg"><?php echo $camp['leads']; ?></span>
                    </div>
                    <div>
                        <span class="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block mb-0.5">Spent Budget</span>
                        <span class="font-bold text-slate-800 text-xs"><?php echo $camp['spent']; ?> / <?php echo $camp['budget']; ?></span>
                    </div>
                    <div>
                        <span class="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block mb-0.5">UTM Click Link</span>
                        <a href="../track_click.php?utm_campaign=<?php echo urlencode($camp['name']); ?>&utm_ad=VariantA&redirect=index.php" target="_blank" class="text-[10px] text-brand-500 underline font-bold">Copy Link</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <div class="flex gap-2">
                    <a href="export.php?<?php echo $exportQuery; ?>&format=csv" class="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-bold transition flex items-center gap-1">
                        CSV Export
                    </a>
                    <a href="export.php?<?php echo $exportQuery; ?>&format=pdf" target="_blank" class="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-bold transition flex items-center gap-1">
                        PDF Export
                    </a>
                </div>
            </div>
            ...
            <form action="meta-ads.php" method="GET" class="row g-2 bg-slate-50 p-3.5 rounded-xl mb-4 border border-slate-200/50">
                <div class="col-md-3">
                    <select name="campaign_name" class="w-full px-2 py-1.5 rounded bg-white border border-slate-200 text-xs">
                ...
                <div class="col-md-2">
                    <input type="date" name="start_date" value="<?php echo $filterStart; ?>" class="w-full px-2 py-1 rounded bg-white border border-slate-200 text-[10px]" placeholder="Start Date">
                </div>
                <div class="col-md-2">
                    <input type="date" name="end_date" value="<?php echo $filterEnd; ?>" class="w-full px-2 py-1 rounded bg-white border border-slate-200 text-[10px]" placeholder="End Date">
                </div>
                
                <div class="col-md-2">
                    <button type="submit" class="w-full py-1.5 bg-slate-800 hover:bg-slate-700 text-white rounded text-xs font-bold transition">
                        Apply
                    </button>
                </div>
            </form>
                ...
                <table class="table table-hover align-middle text-sm">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <?php if (empty($metaLeads)): ?>
                            <tr>
                                <td colspan="4" class="py-6 text-center text-slate-400">No leads captured matching filters.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($metaLeads as $lead): ?>
                                <tr>
                                    <td class="py-3">
                                        ...
                                        <div class="text-[10px] text-slate-400 leading-none"><?php echo htmlspecialchars($lead['email']); ?></div>
                                    </td>
                                    <td class="py-3 font-semibold text-brand-500"><?php echo htmlspecialchars($lead['campaign_name'] ?? 'General Facebook Campaign'); ?></td>
                                    <td class="py-3 text-xs text-slate-500"><?php echo date('d-M-y H:i', strtotime($lead['created_at'])); ?></td>
                                    <td class="py-3">
                                        <?php
                                        $col = 'bg-slate-100 text-slate-600';
                                        if ($lead['status'] === 'new') $col = 'bg-blue-100 text-blue-700';
                                        elseif ($lead['status'] === 'qualified') $col = 'bg-emerald-100 text-emerald-700';
                                        ?>
                ...
                <?php else: ?>
                    <?php foreach ($metaClicks as $click): ?>
                        <div class="p-3 bg-slate-50 rounded-xl border border-slate-100 flex justify-between gap-3 text-xs">
                            <div>
                                <div class="font-bold text-slate-800 truncate max-w-[150px]"><?php echo htmlspecialchars($click['campaign_name']); ?></div>
                                <div class="text-[10px] text-slate-400"><?php echo htmlspecialchars($click['ad_name']); ?> • IP: <?php echo htmlspecialchars($click['ip_address']); ?></div>
                            </div>
                            <div class="text-right flex-shrink-0">
                                <span class="text-[9px] bg-indigo-50 text-brand-500 font-bold px-2 py-0.5 rounded block mb-1">Click Logged</span>
                                <span class="text-[10px] text-slate-400"><?php echo date('H:i', strtotime($click['clicked_at'])); ?></span>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            ...
            <form action="meta-ads.php" method="POST" class="p-6 space-y-4">
                ...
                <div class="flex items-center justify-between p-3 bg-slate-50 border rounded-xl">
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="meta_mock_mode" id="meta_mock_mode" value="1" <?php echo $mockMode ? 'checked' : ''; ?>>
                    </div>
                </div>

                <div>
                    <label for="meta_access_token" class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Meta Marketing Access Token</label>
                    <textarea name="meta_access_token" id="meta_access_token" rows="3" class="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-brand-gold focus:bg-white text-slate-800" placeholder="EAA..."><?php echo htmlspecialchars($settings['meta_access_token'] ?? ''); ?></textarea>
                </div>
                
                <div>
                    <label for="meta_ad_account_id" class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Meta Ad Account ID</label>
                    <input type="text" name="meta_ad_account_id" id="meta_ad_account_id" value="<?php echo htmlspecialchars($settings['meta_ad_account_id'] ?? ''); ?>" class="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-brand-gold focus:bg-white text-slate-800" placeholder="act_...">
                </div>

                <div class="pt-4 border-t flex justify-end gap-3">
                    <button type="submit" class="px-5 py-2 bg-brand-500 hover:bg-brand-600 text-white text-xs font-bold rounded-lg transition shadow">
                        Save Configuration
                    </button>
                    <button type="button" class="px-4 py-2 bg-slate-100 text-slate-700 text-xs font-bold rounded-lg transition" data-bs-dismiss="modal">
                        Close
                    </button>
                </div>
            </form>
